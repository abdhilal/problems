...
<body>
    <div class="container mt-5">
        ...
        @if(Auth::user()->artisan->notifications->isEmpty())
            <div class="alert alert-info">لا توجد إشعارات جديدة.</div>
        @else
            <ul class="list-group">
                @foreach(Auth::user()->artisan->notifications as $notification)
                    <a href="#"
                       onclick="markNotificationAsRead(event, '{{ route('artisans.notifications.markAsRead', $notification->id) }}', '{{ route('problems.show', $notification->data['problem_id']) }}')"
        ...
        <div class="alert alert-info mt-4">
            عدد الإشعارات: {{ Auth::user()->artisan->notifications->count() }}
        </div>
    </div>
